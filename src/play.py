...
class AegisWorldModelWrapper:
    """
    Wraps the underlying WorldModelEnv to monitor state-transition entropy in real-time.
    
    This wrapper intercepts the observation tensor generated by the diffusion process
    at each step. It evaluates the Shannon Entropy of the visual/latent state to detect 
    out-of-distribution (OOD) dynamics or diffusion 'mode collapse' (hallucinations).
    
    If the entropy drops below the critical threshold, it triggers a safety flag, 
    allowing external system-level probes (e.g., eBPF/gVisor) to shunt the agent's 
    execution and prevent deceptive alignment on corrupted physics.
    """
    def __init__(self, env):
        self.env = env
        self.aegis_monitor = AegisEntropyMonitor(threshold_low=3.0)
    ...
    def step(self, action):
        ...
        # Intercept the observation tensor
        obs = step_results[0] if isinstance(step_results, tuple) else step_results
        
        # Monitor for visual/latent entropy collapse
        if isinstance(obs, torch.Tensor):
            self.aegis_monitor.check_entropy_collapse(obs)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--aegis", action="store_true", help="Enable Aegis Entropy Monitor on World Model.")
...
def prepare_play_mode(cfg: DictConfig, args: argparse.Namespace) -> Tuple[PlayEnv, Keymap, ActionNames]:
    ...
    # Wrap with Aegis to monitor generation stability during play
    wm_env = AegisWorldModelWrapper(wm_env)
